...
import { Plus, Trash2, Pencil, X, Newspaper } from "lucide-react";
import { getCollection, saveCollection, logActivity, slugify } from "@/lib/admin-store";
import { blogPosts } from "@/data/blog";
import { images } from "@/lib/images";
import { BlogPost, Expert } from "@/lib/types";
...
export function ExpertBlogSection({ expert }: { expert: Expert }) {
  ...
  useEffect(() => {
    getCollection<BlogPost>(STORAGE_KEY, blogPosts).then(setAllPosts);
  }, []);

  const myPosts = (allPosts ?? []).filter((p) => p.authorExpertSlug === expert.slug);
  const totalPages = totalPagesFor(myPosts.length);
  ...
  function removePost(slug: string) {
    ...
    persist(allPosts.filter((p) => p.slug !== slug));
    logActivity(`${expert.name} deleted a blog post: ${slug}`);
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const post: BlogPost = {
      ...
      author: { name: expert.name, role: expert.title, image: expert.image },
      authorExpertSlug: expert.slug,
    ...
    if (editingSlug) {
      persist(allPosts.map((p) => (p.slug === editingSlug ? post : p)));
      logActivity(`${expert.name} updated a blog post: ${post.title}`);
    } else {
      persist([post, ...allPosts]);
      setPage(1);
      logActivity(`${expert.name} published a new blog post: ${post.title}`);
    }
    setShowForm(false);
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="font-serif-display text-2xl text-plum-900">My Posts</h2>
        <button
          onClick={openCreate}
      ...
      {myPosts.length === 0 && (
        <div className="rounded-2xl border border-dashed border-plum/20 p-10 text-center text-plum-soft">
          <Newspaper className="mx-auto mb-3 size-6 text-plum-soft" strokeWidth={1.6} />
          No posts yet. Share your knowledge with an article for your clients.
        </div>
      )}
      ...
      {myPosts.length > 0 && (
        <Pagination page={page} totalPages={totalPages} onPageChange={setPage} totalItems={myPosts.length} itemLabel="post" />
      )}
    </div>
  );
}
